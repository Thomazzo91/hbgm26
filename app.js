/* Mattor — lopp, lag, punkter och körvägar. Ingen server. */
(function (global) {
  ...
  async function recalcTeam(team, onProgress) {
    ...
    const iga = await buildPath(igaStops);
    team.modes = {
      kortast: { km: kort.km, min: kort.min, gpx: "", stops: kortStops, legs: kort.legs, track: kort.track },
      iga: { km: iga.km, min: iga.min, gpx: "", stops: igaStops, legs: iga.legs, track: iga.track }
    };
    return team;
  }
  ...
  function gpxFor(team, modeId) {
    ...
    const name = (team.name || "Lag") + (modeId === "iga" ? " (igång-ordning)" : "");
