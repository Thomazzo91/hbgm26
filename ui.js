(function () {
  ...
  function renderNow(g) {
    ...
    if (!s) {
      nowEl.innerHTML = "<p class=\"who\">Inga punkter i det här laget. Öppna Redigera lopp.</p>";
      return;
    }
  ...
  function renderEditor() {
    ...
    editorBody.innerHTML = `
      ...
      <label>Lag (det som är Söder / Mitten / Norr i HBGM)</label>
      ...
      <div class="edit-actions">
        <button type="button" class="btn" id="addTeam">+ Lag</button>
        <button type="button" class="btn btn-danger" id="delTeam">Ta bort lag</button>
      </div>
    ...
    teams().forEach((t) => {
      const b = document.createElement("button");
      b.type = "button";
      b.className = "btn" + (t.id === editTeamId ? " btn-accent" : "");
      b.textContent = t.name;
    ...
    editorBody.querySelector("#teamColor").addEventListener("change", saveEditorFields);
  ...
  function renderPointForm(i) {
    ...
    box.innerHTML = `
      ...
      <div class="row">
        <div><label>Igång</label><input id="pIga" value="${esc(p.iga)}" placeholder="07:30" /></div>
        <div><label>Första</label><input id="pForsta" value="${esc(p.forsta)}" /></div>
        <div><label>Sista</label><input id="pSista" value="${esc(p.sista)}" /></div>
      </div>
  ...
  function readPointForm(i) {
    ...
    pts[i].iga = box.querySelector("#pIga").value.trim();
    pts[i].forsta = box.querySelector("#pForsta").value.trim();
    pts[i].sista = box.querySelector("#pSista").value.trim();
  ...
  function deleteTeam() {
    const ev = currentEvent();
    if (ev.teams.length < 2) { showToast("Minst ett lag behövs"); return; }
    if (!confirm("Ta bort laget " + teamById(editTeamId).name + "?")) return;
  ...
  document.getElementById("gpxBtn").addEventListener("click", () => {
    ...
    download((t.name || "lag") + ".gpx", xml, "application/gpx+xml");
  });
